...
async function start_game(io: any, gameState: GameState, room_index: string) {
  ...
  let current_room = rooms.find((r: Room) => {
    return r.index == room_index;
  });

  /*
    5 seconds to wait for the turn end
    7 seconds to chosee one word of the 3 words. 
    3 seconds to start drawing.
    if the player don't drawing in 30 seconds, the player is AFK.
  */
  ...
  if (current_room) {
    ...
    io.in(room_index).emit("update_round", {
...
async function start_turn(io: any, gameState: GameState, room_index: string) {
  try {
    let rooms = gameState.rooms;
    let current_room: any = rooms.find((r) => {
      return r.index == room_index;
    });
    ...
    if (current_room) {
      ...
      console.log(
        "this is ROOM FROM Start turn: " + JSON.stringify(current_room)
      );
      ...
      for (let socket in io.sockets.in(room_index).connected) {
        if (
          io.sockets.in(room_index).connected[socket].username ==
          painter_username
        ) {
          console.log("this is the painter_username: " + painter_username);
          console.log(
            "this is the socket id: " +
              io.sockets.in(room_index).connected[socket].id
          );
      ...
      io.in(current_room).emit("chat_evt", {
      ...
      io.in(room_index).emit("reveal_word_length", {
        word_length: current_turn.word.length,
      });
      io.to(painter_socket_id).emit("reveal_word", { word: current_turn.word });
      ...
      if (get_ready_sec === 0) {
        io.in(room_index).emit("start_drawing");
    ...
    while (!stop_time) {
      ...
      if (
        ...
        if (current_room.current_round != 3 && isEndOfRound) {
          ...
          io.in(room_index).emit("reveal_word", {
            word: current_room.current_turn.word,
          });
          io.in(room_index).emit("show_scoreboard", {
            is_final: false,
          });
          ...
          let winner = current_room.players.sort(function(a, b) {
          ...
          io.in(room_index).emit("score_change", {
        ...
        if (current_turn.countdown === next_reveal_sec) {
          ...
          do {
            ...
            result = current_turn.revealed.find(
              (letter_pos) => letter_pos == pos_to_reveal
            );
          } while (result);
          current_turn.revealed.push(pos_to_reveal);
          // show hints every freq seconds
          io.in(current_room.index).emit("reveal_letter", {
            letter: word[pos_to_reveal].toUpperCase(),
            pos: pos_to_reveal,
          });
          next_reveal_sec -= freq;
        }
        io.in(current_room.index).emit("turn_countdown_sec", {
